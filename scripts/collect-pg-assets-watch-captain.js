/**
 * PG 赏金船长 (game 54) — 边玩边录资源
 *
 * 用法：
 * 1. 打开官方试玩页，等主界面出来
 * 2. F12 -> Console，粘贴本脚本回车
 * 3. 玩游戏 3~5 分钟（转轴、免费局、大赢）
 * 4. 控制台输入： PG_FINISH_COLLECT()
 * 5. 把下载文件放进 scripts/赏金船长/ 后运行 deploy-pg-captain-assets.py
 */
(function () {
  const GAME_ID = '54';
  const DELAY_MS = 200;
  ...
  const track = (url) => {
    ...
    seen.add(url);
    queue.push(url);
    console.log(`[PG54+] ${queue.length} ${url.slice(0, 100)}`);
  };
  ...
  try {
    ...
    window.__PG54_OBS__ = obs;
  } catch (err) {
    console.warn('[PG54] PerformanceObserver 不可用', err);
  }
  ...
  window.PG_FINISH_COLLECT = async () => {
    const httpUrls = [...seen].filter((u) => /^https?:\/\//i.test(u));
    const manifest = {
      collectedAt: new Date().toISOString(),
      page: location.href,
      gameId: GAME_ID,
      mode: 'watch',
      summary: { http: httpUrls.length },
      items: [],
    };
    ...
    for (let i = 0; i < httpUrls.length; i++) {
      ...
      try {
        const meta = await downloadHttp(url, filename);
    ...
    console.log('[PG54] 完成。文件放进 scripts/赏金船长/ 后运行 deploy-pg-captain-assets.py');
    return manifest.summary;
  };

  console.log('[PG54] 录制已启动。完成后输入 PG_FINISH_COLLECT()');
})();
