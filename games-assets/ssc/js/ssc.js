...
$(function () {
  ...
  // V66 快乐时时彩：2 分钟一期 = 1:56 下注倒计时 + 4s 准备中
  const INTERVAL = 120;
  const PREP_SEC = 4;
  ...
  function initKeypad() {
    ...
    let html = '';
    rows.forEach(row => {
      html += `<button type="button" class="ssc-key ssc-key--pos" data-key="${row.left}">${row.left}</button>`;
      if (row.amtWide) {
        html += `<button type="button" class="ssc-key ssc-key--amt" data-key="${row.mid[0]}">${row.mid[0]}</button>`;
      } else {
        row.mid.forEach(k => {
  ...
  function renderPlayPanel() {
    ...
    if (cat.subs && cat.subs.length) {
      $sub.removeClass('is-hidden').prop('hidden', false).html(
        cat.subs.map((s, i) =>
          `<button type="button" class="ssc-play-sub__btn${i === playSubIdx ? ' is-active' : ''}" data-sub="${i}">${s}</button>`
        ).join('')
      );
    } else {
    ...
    if (cat.mode === 'positionDigit' || cat.mode === 'segDigit') {
      ...
      html = cat.shapes.map(shape => {
        ...
        return `<button type="button" class="ssc-play-item${active ? ' is-active' : ''}" data-play="${play}"><span>${shape}</span><small>${SHAPE_ODDS[shape]}</small></button>`;
      }).join('');
  ...
  function initPlaySheetEvents() {
    ...
    $('#playSub').on('click', '.ssc-play-sub__btn', function () {
      playSubIdx = +$(this).data('sub');
      playPickedDigits = [];
      $('#playSub .ssc-play-sub__btn').removeClass('is-active');
      $(this).addClass('is-active');
      renderPlayPanel();
      updatePlaySelected();
    });
