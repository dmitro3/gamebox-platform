...
$(function () {
  ...
  const PLAY_CATEGORIES = [
    ...
    {
      id: 'position', title: '定位车号', hint: '猜指定名次车号 · 可多选 · 赔率 9.8',
      subs: null,
      mode: 'positionNum',
      odds: ODDS.position
    }
    ...
    if (cat.id === 'gySide') {
      ...
      html = LHH_PAIRS.map(([a, b]) => {
        ...
        const cells = ['龙', '虎'].map(pick =>
          playItemHtml(pr + pick, pick, ODDS.dragon, 'bjsc-play-item--lhh')
        ).join('');
        return `<div class="bjsc-play-lhh-row"><span class="bjsc-play-lhh-row__label">${pr}</span>${cells}</div>`;
      }).join('');
      ...
      html = POS_NAMES.map(name => {
        ...
        return `<div class="bjsc-play-pos-block">
          <div class="bjsc-play-pos-block__head">
            <span class="bjsc-play-pos-block__title">${name}</span>
        ...
        <div class="bjsc-predict-hero">
          ...
          <div class="bjsc-predict-hero__main">
            <div class="bjsc-predict-hero__tags">
              <span class="bjsc-predict-card__tag ${predictTagClass(champBig.pick)} is-clickable" data-predict-play="冠军${champBig.pick}" role="button" tabindex="0">冠军${champBig.pick}</span>
              <span class="bjsc-predict-card__tag ${predictTagClass(champOdd.pick)} is-clickable" data-predict-play="冠军${champOdd.pick}" role="button" tabindex="0">冠军${champOdd.pick}</span>
            </div>
            <div class="bjsc-predict-hero__note">冠军倾向 ${champBig.pick}${champOdd.pick}，${champBig.reason}</div>
          </div>
        </div>`;
