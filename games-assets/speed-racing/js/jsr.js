    ...
    if (cat.id === 'gySide') {
      ...
      html = LHH_PAIRS.map(([a, b]) => {
        ...
        const cells = ['龙', '虎'].map(pick =>
          playItemHtml(pr + pick, pick, ODDS.dragon, 'jsr-play-item--lhh')
        ).join('');
        return `<div class="jsr-play-lhh-row"><span class="jsr-play-lhh-row__label">${pr}</span>${cells}</div>`;
      }).join('');
      ...
      html = POS_NAMES.map(name => {
        ...
        return `<div class="jsr-play-pos-block">
          <div class="jsr-play-pos-block__head">
            <span class="jsr-play-pos-block__title">${name}</span>
        ...
        <div class="jsr-predict-hero">
          ...
          <div class="jsr-predict-hero__main">
            <div class="jsr-predict-hero__tags">
              <span class="jsr-predict-card__tag ${predictTagClass(champBig.pick)} is-clickable" data-predict-play="冠军${champBig.pick}" role="button" tabindex="0">冠军${champBig.pick}</span>
              <span class="jsr-predict-card__tag ${predictTagClass(champOdd.pick)} is-clickable" data-predict-play="冠军${champOdd.pick}" role="button" tabindex="0">冠军${champOdd.pick}</span>
            </div>
            <div class="jsr-predict-hero__note">冠军倾向 ${champBig.pick}${champOdd.pick}，${champBig.reason}</div>
          </div>
        </div>`;
