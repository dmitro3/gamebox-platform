/**
 * 极速赛车 · 聊天投注房
 * 界面对齐时时彩；玩法对齐官方 PK10/极速赛车：
 * 名次两面 / 龙虎 / 冠亚和值 / 定位车号
 * BUILD 79 — 与 index.html ?v= 保持一致
 */
$(function () {
  // 总周期 55s；准备中 = 揭晓结果卡+中奖核对 ≈1.4s → 取 2s；倒计时从 00:53 起
  const INTERVAL = 55;
  const PREP_SEC = 2;
  ...
  function avatarHtml(name, seed) {
    ...
    return `<div class="jsr-msg__avatar jsr-msg__avatar--gen" style="--av-hue:${hue}"><span>${ch}</span></div>`;
  }
  ...
  function betLogKey() { return 'jsr_bet_log_' + user.uid; }
  ...
  function saveUserBetLog(list) {
    ...
    try { window.dispatchEvent(new CustomEvent('jsr-bet-log-updated')); } catch (e) { /* */ }
  }
  function appendUserBetLog(issue, play, amount) {
    const log = loadUserBetLog();
    let group = log.find(g => g.issue === issue);
    if (!group) {
      group = { game: 'speed-racing', issue, createdAt: new Date().toISOString(), settled: false, bets: [], totalAmount: 0, totalProfit: 0 };
      log.unshift(group);
    }
  ...
  function buildBetListVerifyHtml(issue) {
    ...
    if (!book || !Object.keys(book).length) {
      return `<div class="jsr-bot-card jsr-bot-card--bets"><div class="jsr-bot-card__head"><span class="jsr-bot-card__title">注单核对</span><span class="jsr-bot-card__issue">${issue}</span></div><div class="jsr-bot-empty">本期暂无下注</div></div>`;
    }
    const names = Object.keys(book);
    const users = names.map(name => {
      const bets = book[name];
      const total = bets.reduce((s, b) => s + b.amount, 0);
      const tags = bets.map(b => `<span class="jsr-bot-tag">${b.play}/${b.amount}</span>`).join('');
      return `<div class="jsr-bot-user${name === user.name ? ' is-me' : ''}"><div class="jsr-bot-user__head"><span class="jsr-bot-user__name">${name}</span><span class="jsr-bot-user__sum">${total}</span></div><div class="jsr-bot-user__tags">${tags}</div></div>`;
    }).join('');
    return `<div class="jsr-bot-card jsr-bot-card--bets"><div class="jsr-bot-card__head"><span class="jsr-bot-card__title">注单核对</span><span class="jsr-bot-card__issue">${issue}</span></div><div class="jsr-bot-card__body is-full">${users}</div></div>`;
  }
  ...
  function buildWinListVerifyHtml(issue, settled) {
    ...
    let body = winners.length ? winners.map(name => {
      ...
      const tags = winRows.map(r => `<span class="jsr-bot-tag jsr-bot-tag--win">${r.play}/${r.amount}</span>`).join('');
      return `<div class="jsr-bot-user"><div class="jsr-bot-user__head"><span class="jsr-bot-user__name">${name}</span><span class="jsr-bot-user__profit ${totalProfit >= 0 ? 'pos' : 'neg'}">${fmtProfit(totalProfit)}</span></div><div class="jsr-bot-user__tags">${tags}</div></div>`;
    }).join('') : '<div class="jsr-bot-empty">本期暂无中奖</div>';
    return `<div class="jsr-bot-card jsr-bot-card--win"><div class="jsr-bot-card__head"><span class="jsr-bot-card__title">中奖核对</span><span class="jsr-bot-card__issue">${issue}</span></div><div class="jsr-bot-card__body is-full">${body}</div></div>`;
  }
  ...
  function ballHtml(n, cls) {
    ...
    return `<span class="jsr-ball${c}" data-n="${n}"><img class="jsr-ball__img" src="${file}" alt="${n}" loading="lazy"></span>`;
  }
  ...
  function renderCountdownDigits(sec) {
    ...
    const ss = pad(s % 60, 2);
    const cells = $('#cdDigits .jsr-timer__cell');
    if (cells.length >= 4) {
  ...
  function getStats(uid) {
    try { return JSON.parse(localStorage.getItem('jsr_stats_' + uid)) || { turnover: 0, winloss: 0, rebate: 0 }; } catch (e) { return { turnover: 0, winloss: 0, rebate: 0 }; }
  }
  function saveStats(uid, s) { localStorage.setItem('jsr_stats_' + uid, JSON.stringify(s)); }
  ...
  function renderDrawTable() {
    $('#drawTableBody').html(historyRows.slice(0, HISTORY_SHOW).map(r => `
      <tr><td class="jsr-draw-table__issue">${r.issue}</td>
  ...
  function resultCardHtml(nums, issue) {
    ...
    const lh = lhRow(nums);
    return JsrResultCard.build(nums, issue, meta, lh);
  }
  ...
  function appendMsg(opts) {
    ...
    const avatar = opts.avatarHtml || `<div class="jsr-msg__avatar"><img src="${fixAvatar(opts.avatar)}" alt=""></div>`;
    const metaOther = `<div class="jsr-msg__meta"><span class="jsr-msg__name">${opts.name}</span><span class="jsr-msg__time">${time}</span></div>`;
    const metaSelf = `<div class="jsr-msg__meta"><span class="jsr-msg__time">${time}</span><span class="jsr-msg__name">${opts.name}</span></div>`;
    const bubbleCls = opts.robot ? 'jsr-msg__bubble jsr-msg__bubble--bot' : 'jsr-msg__bubble';
    const bubble = `<div class="${bubbleCls}">${opts.html}</div>`;
    const html = opts.self
      ? `<div class="jsr-msg is-self"><div class="jsr-msg__top">${metaSelf}${avatar}</div>${bubble}</div>`
      : `<div class="jsr-msg${opts.robot ? ' is-robot' : ''}">${avatar}<div class="jsr-msg__body">${metaOther}${bubble}</div></div>`;
    $('#chatFeed').append(html);
    scrollChat();
  }

  function closeNoticeHtml(issue) {
    return `<div class="jsr-bot-close-line"><div class="jsr-bot-close-line__bar"><span class="jsr-bot-close-line__deco"></span><span class="jsr-bot-close-line__text">封 盘 线</span><span class="jsr-bot-close-line__deco"></span></div><div class="jsr-bot-close-line__sub">第 <span class="jsr-bot-close-line__issue">${issue}</span> 期 · 停止下注 · 即将开奖</div></div>`;
  }
  ...
  function pushRobotResult(nums, issue, syncReveal) {
    ...
    setTimeout(() => {
      ...
      const card = $('#chatFeed .jsr-result-card').last()[0];
      if (card) JsrResultCard.bindStage(card);
    }, resultDelay);
    setTimeout(() => {
  ...
  function initKeypad() {
    ...
    let html = '';
    for (let r = 0; r < 4; r++) {
      html += `<button type="button" class="jsr-key jsr-key--side" data-key="${left[r]}">${left[r]}</button>`;
      mid[r].forEach(k => { html += `<button type="button" class="jsr-key" data-key="${k}">${k}</button>`; });
      const rk = right[r];
      const cls = rk === '删除' ? ' jsr-key--del' : (rk === '冠亚' ? ' jsr-key--gy' : ' jsr-key--side');
      html += `<button type="button" class="jsr-key${cls}" data-key="${rk}">${rk}</button>`;
    }
    $('#keyGrid').html(html);
  }
  ...
  function clearPlayAmountChipActive() {
    $('#playAmountChips .jsr-play-foot__chip').removeClass('is-active');
  }
  ...
  function initPlaySheet() {
    $('#playNav').html(PLAY_CATEGORIES.map((c, i) =>
      `<button type="button" class="jsr-play-nav__item${i === playCatIdx ? ' is-active' : ''}" data-idx="${i}">${c.title}</button>`
    ).join(''));
    $('#playAmountChips').html(AMOUNT_CHIPS.map(a =>
      `<button type="button" class="jsr-play-foot__chip${a === 100 ? ' is-active' : ''}" data-amt="${a}">${a}</button>`
    ).join(''));
  }

  function playItemHtml(play, label, odds, extraCls) {
    const cls = ['jsr-play-item', extraCls, isPlaySelected(play) ? 'is-active' : ''].filter(Boolean).join(' ');
  ...
  function playNumItemHtml(posName, n) {
    ...
    const cls = ['jsr-play-item', 'jsr-play-item--num', isPlaySelected(play) ? 'is-active' : ''].filter(Boolean).join(' ');
  ...
  function renderPlayPanel() {
    ...
    if (cat.subs && cat.subs.length) {
      $sub.removeClass('is-hidden').prop('hidden', false).html(
        cat.subs.map((s, i) => `<button type="button" class="jsr-play-sub__btn${i === playSubIdx ? ' is-active' : ''}" data-sub="${i}">${s}</button>`).join('')
      );
    } else {
    ...
    if (cat.id === 'gySide') {
      ...
      html = items.map(it => {
        ...
        return playItemHtml(it.play, short, it.odds, 'jsr-play-item--side');
      }).join('');
    } else if (cat.id === 'posSide' && cat.sideGroups) {
      gridMode = 'is-pos-num';
      html = cat.sideGroups.map(g => {
        const cells = ['大', '小', '单', '双'].map(side =>
          playItemHtml(g.prefix + side, side, g.odds, 'jsr-play-item--side')
        ).join('');
        return `<div class="jsr-play-pos-block jsr-play-pos-block--side">
          <div class="jsr-play-pos-block__head">
            <span class="jsr-play-pos-block__title">${g.label}</span>
            <span class="jsr-play-pos-block__hint">大小单双 · ${g.odds}</span>
          </div>
          <div class="jsr-play-pos-block__side">${cells}</div>
        </div>`;
      }).join('');
    } else if (cat.id === 'lhh') {
      gridMode = 'is-lhh';
      html = LHH_PAIRS.map((_, i) => {
        ...
        const cells = ['龙', '虎'].map(pick =>
          playItemHtml(n + pick, pick, ODDS.dragon, 'jsr-play-item--lhh')
        ).join('');
        return `<div class="jsr-play-lhh-row"><span class="jsr-play-lhh-row__label">${n}组</span>${cells}</div>`;
      }).join('');
      ...
      html = Array.from({ length: 10 }, (_, i) => {
        ...
        return `<div class="jsr-play-pos-block">
          <div class="jsr-play-pos-block__head">
            <span class="jsr-play-pos-block__title">赛道${t}</span>
            <span class="jsr-play-pos-block__hint">选该车号 · 赔率 ${cat.odds}</span>
          </div>
          <div class="jsr-play-pos-block__nums">${nums}</div>
        </div>`;
      }).join('');
    } else {
      const items = typeof cat.items === 'function' ? cat.items() : cat.items;
    ...
    $grid.attr('class', `jsr-play-panel__grid${gridMode ? ' ' + gridMode : ''}`).html(html);
    $grid.scrollTop(0);
  }
  ...
  function layoutPlayOverlay() {
    ...
    document.documentElement.style.setProperty('--jsr-play-top', top + 'px');
  }
  function openPlaySheet(open) {
    ...
    if (open) {
      ...
      syncPlayAmountUI();
      clearPlayAmountChipActive();
      $('#playAmountChips .jsr-play-foot__chip[data-amt="100"]').addClass('is-active');
      refreshStats();
  ...
  function initPlaySheetEvents() {
    $('#playNav').on('click', '.jsr-play-nav__item', function () {
      playCatIdx = +$(this).data('idx');
      playSubIdx = 0;
      $('#playNav .jsr-play-nav__item').removeClass('is-active');
      $(this).addClass('is-active');
      renderPlayPanel();
    });
    $('#playSub').on('click', '.jsr-play-sub__btn', function () {
      playSubIdx = +$(this).data('sub');
      $('#playSub .jsr-play-sub__btn').removeClass('is-active');
      $(this).addClass('is-active');
      renderPlayPanel();
    });
    $('#playGrid').on('click', '.jsr-play-item', function () {
      const play = $(this).data('play');
      togglePlaySelect(play);
      $(this).toggleClass('is-active', isPlaySelected(play));
      updatePlaySelected();
    });
    $('#playAmountChips').on('click', '.jsr-play-foot__chip', function () {
  ...
  function longStreakBar(count) {
    ...
    return `<div class="jsr-long-item__bar"><span style="width:${pct}%"></span></div>`;
  }
  ...
  function renderLongDragonPanel() {
    ...
    if (!list.length) {
      ...
      $('#longBody').html(`
        <div class="jsr-long-empty">
          <div class="jsr-long-empty__icon">◎</div>
          <div class="jsr-long-empty__text">当前没有 2 期以上的长龙</div>
          <div class="jsr-long-empty__hint">开奖后会自动更新</div>
        </div>`);
      return;
    }
    const top = list[0];
    ...
    const hero = `
      <div class="jsr-long-hero ${longTierClass(top.count)}">
        <div class="jsr-long-hero__label">当前最长</div>
        <div class="jsr-long-hero__main">
          <span class="jsr-long-hero__tag ${longTagClass(topParts.tag)}">${topParts.tag}</span>
          <span class="jsr-long-hero__text">${topParts.full}</span>
          <span class="jsr-long-hero__num">${top.count}</span>
          <span class="jsr-long-hero__unit">期</span>
        </div>
      </div>`;
    const rows = list.map((item, idx) => {
      ...
      return `
        <div class="jsr-long-item ${tier}">
          <div class="jsr-long-item__rank">${idx + 1}</div>
          <div class="jsr-long-item__tag ${longTagClass(parts.tag)}">${parts.tag}</div>
          <div class="jsr-long-item__info">
            <div class="jsr-long-item__name">${parts.group}<span class="sep">·</span>${parts.full}</div>
            ${longStreakBar(item.count)}
          </div>
          <div class="jsr-long-item__count">
            <b>${item.count}</b><small>连</small>
          </div>
        </div>`;
    }).join('');
    $('#longBody').html(hero + `<div class="jsr-long-list">${rows}</div>`);
  }
  ...
  function predictCardHtml(name, data, play) {
    ...
    return `<div class="jsr-predict-card is-clickable"${playAttr} role="button" tabindex="0">
      <div class="jsr-predict-card__name">${name}</div>
      <div class="jsr-predict-card__pick">
        <span class="jsr-predict-card__tag ${predictTagClass(data.pick)}">${data.pick}</span>
        <span class="jsr-predict-card__conf">${pct}%</span>
      </div>
      <div class="jsr-predict-card__reason">${data.reason}</div>
      <div class="jsr-predict-card__bar"><span style="width:${pct}%"></span></div>
    </div>`;
  }
  ...
  function renderPredictPanel() {
    ...
    if (!rows.length) {
      $('#predictBody').html(`
        <div class="jsr-long-empty">
          <div class="jsr-long-empty__icon">◎</div>
          <div class="jsr-long-empty__text">暂无开奖数据</div>
          <div class="jsr-long-empty__hint">等待开奖后生成预测</div>
        </div>`);
      return;
    }
    ...
    if (predictTab === 'side') {
      ...
      const hero = `
        <div class="jsr-predict-hero">
          <div class="jsr-predict-hero__label">核心参考</div>
          <div class="jsr-predict-hero__main">
            <div class="jsr-predict-hero__tags">
              <span class="jsr-predict-card__tag ${predictTagClass(champBig.pick)} is-clickable" data-predict-play="1${champBig.pick}" role="button" tabindex="0">赛道1${champBig.pick}</span>
              <span class="jsr-predict-card__tag ${predictTagClass(champOdd.pick)} is-clickable" data-predict-play="1${champOdd.pick}" role="button" tabindex="0">赛道1${champOdd.pick}</span>
            </div>
            <div class="jsr-predict-hero__note">赛道1倾向 ${champBig.pick}${champOdd.pick}，${champBig.reason}</div>
          </div>
        </div>`;
      $('#predictBody').html(hero + `<div class="jsr-predict-grid">${cards.join('')}</div>`);
      return;
    }
    ...
    if (predictTab === 'num') {
      const cols = POS_NAMES.map((pos, idx) => {
        ...
        const hotHtml = stat.hot.length
          ? stat.hot.map(x =>
            `<span class="jsr-ball-digit is-clickable" data-predict-play="${pos}${x.d}" role="button" tabindex="0">${x.d}</span>`
          ).join('')
          : '<span class="jsr-predict-card__reason">—</span>';
        const coldHtml = stat.cold.map(x =>
          `<span class="jsr-ball-digit is-cold is-clickable" data-predict-play="${pos}${x.d}" role="button" tabindex="0">${x.d}</span>`
        ).join('');
        return `<div class="jsr-predict-digit-col">
          <div class="jsr-predict-digit-col__pos">${pos}</div>
          <div class="jsr-predict-digit-col__label">热号</div>
          <div class="jsr-predict-digit-balls">${hotHtml}</div>
          <div class="jsr-predict-digit-col__label">遗漏</div>
          <div class="jsr-predict-digit-balls">${coldHtml}</div>
        </div>`;
      }).join('');
      $('#predictBody').html(`
        <div class="jsr-predict-hero">
          <div class="jsr-predict-hero__label">定位参考</div>
          <div class="jsr-predict-hero__note">近 ${Math.min(PREDICT_WINDOW, rows.length)} 期各名次热号与遗漏号，可作定位车号参考</div>
        </div>
        <div class="jsr-predict-digit-grid">${cols}</div>`);
      return;
    }
    ...
    const hero = `
      <div class="jsr-predict-hero">
        <div class="jsr-predict-hero__label">冠亚参考</div>
        <div class="jsr-predict-hero__main">
          <div class="jsr-predict-hero__tags">
            <span class="jsr-predict-card__tag ${predictTagClass(sizeData.pick)} is-clickable" data-predict-play="冠亚${sizeData.pick}" role="button" tabindex="0">冠亚${sizeData.pick}</span>
            <span class="jsr-predict-card__tag ${predictTagClass(oddData.pick)} is-clickable" data-predict-play="冠亚${oddData.pick}" role="button" tabindex="0">冠亚${oddData.pick}</span>
          </div>
          <div class="jsr-predict-hero__note">${sizeData.reason}</div>
        </div>
      </div>`;
    $('#predictBody').html(hero
      + `<div class="jsr-predict-grid">${predictCardHtml('冠亚 · 大小', sizeData, '冠亚' + sizeData.pick)}${predictCardHtml('冠亚 · 单双', oddData, '冠亚' + oddData.pick)}</div>`
      + `<div class="jsr-predict-hero" style="margin-top:12px"><div class="jsr-predict-hero__label">热和值</div></div>`
      + `<div class="jsr-predict-grid">${sumCards}</div>`
      + `<div class="jsr-predict-hero" style="margin-top:12px"><div class="jsr-predict-hero__label">龙虎走势</div></div>`
      + `<div class="jsr-predict-grid">${lhhCards}</div>`);
  }
  ...
  function openRulesSheet(open) {
    if (open) {
      ...
      const html =
        '<p class="jsr-rules-intro">' + tip + '</p>' +
        (rows
          ? '<table class="jsr-rules-table"><thead><tr><th>玩法</th><th>说明</th></tr></thead><tbody>' + rows + '</tbody></table>'
          : '<p class="jsr-rules-intro">点击右侧或输入栏「玩法」可点选下注；封盘后不可下注。</p>');
      $('#rulesBody').html(html);
  ...
  function goBetRecords() {
    ...
    App.go('../../client-app/pages/bet-records/bet-records.html' + q + sep + 'game=speed-racing');
  }
...
$('#btnSlip').on('click', () => {
  ...
  $('#keyGrid').on('click', '.jsr-key', function () { insertInput($(this).data('key')); updateComposerAction(); });
  ...
  $('#plusMenu').on('click', 'button', function () {
    ...
    if (routes[key]) {
      let go = routes[key] + q;
      if (key === 'bet-records') go += (go.includes('?') ? '&' : '?') + 'game=speed-racing';
      App.go(go);
    } else App.toast('即将开放');
  });
  $(document).on('click', function (e) {
    ...
    if (!$(e.target).closest('#keypad, #chatInput, .jsr-composer').length && $('#keypad').is(':visible')) {
      if (!$(e.target).closest('.jsr-plus-menu, #plusBtn, .jsr-menu-mask').length) showKeypad(false);
    }
  });
  updateComposerAction();
});
