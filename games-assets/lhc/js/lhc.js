...
$(function () {
  ...
  function resultCardHtml(nums, issue) {
    ...
    return `<div class="lhc-result-card"><div class="lhc-result-card__head"><span class="lhc-result-card__title">六合彩</span>
      <div class="lhc-result-card__order">${nums.map(n=>ballHtml(n,'sm')).join('')}</div></div>
      <div class="lhc-result-card__foot" style="grid-template-columns:1fr 1fr 1fr">
        <div class="lhc-result-stat"><span class="k">期号</span><span class="v">No.${String(issue).slice(-6)}</span></div>
        <div class="lhc-result-stat"><span class="k">总和</span><span class="v">${meta.text}</span></div>
        <div class="lhc-result-stat"><span class="k">特码色波</span><span class="v">${meta.wave}</span></div>
      </div></div>`;
  }
  ...
  function fmtIssue8(seq) {
    return '3406' + pad(Math.max(1, seq) % 10000, 4);
  }
  function fmtTime(d) {
  ...
  $('#playSheetBody').html(PLAY_ITEMS.map(p =>
    `<button type="button" class="lhc-play-item" data-play="${p}">${p}</button>`
  ).join(''));
  ...
  $('#playSheetBody').on('click', '.lhc-play-item', function () {
