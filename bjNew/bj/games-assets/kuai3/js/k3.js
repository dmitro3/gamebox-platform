...
$(function () {
  ...
  function ballHtml(n, cls) {
    const c = cls ? ' ' + cls : '';
    return `<span class="k3-ball-img k3-ball-dice${c}">⚅${n}</span>`.replace('⚅','');
  }
  ...
  function renderDrawBar(row) {
    ...
    $('#barBalls').html(row.nums.map(n => `<span class="k3-dice xs">${n}</span>`).join(''));
  ...
  function renderDrawTable() {
    $('#drawTableBody').html(historyRows.slice(0, HISTORY_SHOW).map(r => `
      ...
      ${r.nums.map(n=>`<td><span class="k3-dice tbl">${n}</span></td>`).join('')}
  ...
  function resultCardHtml(nums, issue) {
    ...
    return `<div class="k3-result-card"><div class="k3-result-card__head"><span class="k3-result-card__title">极速快三</span>
      <div class="k3-result-card__order">${nums.map(n=>`<span class="k3-dice sm">${n}</span>`).join('')}</div></div>
      <div class="k3-result-card__foot"><div class="k3-result-stat"><span class="k">期号</span><span class="v">No.${String(issue).slice(-6)}</span></div>
      <div class="k3-result-stat"><span class="k">和值</span><span class="v">${meta.text}</span></div>
      <div class="k3-result-stat"><span class="k">形态</span><span class="v">${lhRow(nums)}</span></div></div></div>`;
  }
  ...
  function fmtIssue8(seq) {
    return '3411' + pad(Math.max(1, seq) % 10000, 4);
  }
  function fmtTime(d) {
  ...
  function getStats(uid) {
    const key = 'k3_stats_' + uid;
    let s = null;
    try { s = JSON.parse(localStorage.getItem(key)); } catch (e) { /* */ }
    return s || { turnover: 0, winloss: 0, rebate: 0 };
  }
  function saveStats(uid, s) {
    localStorage.setItem('k3_stats_' + uid, JSON.stringify(s));
  }
  ...
  function appendMsg(opts) {
    ...
    const avatar = `<div class="k3-msg__avatar"><img src="${fixAvatar(opts.avatar)}" alt=""></div>`;
    const meta = `<div class="k3-msg__meta">
            <span class="k3-msg__name">${opts.name}</span>
            <span class="k3-msg__time">${time}</span>
          </div>`;
    const bubble = `<div class="k3-msg__bubble">${opts.html}</div>`;
    const html = isSelf
      ? `<div class="k3-msg is-self">
        <div class="k3-msg__top">${meta}${avatar}</div>
        ${bubble}
      </div>`
      : `<div class="k3-msg${isRobot ? ' is-robot' : ''}">
        ${avatar}
        <div class="k3-msg__body">
          ${meta}
          ${bubble}
        </div>
      </div>`;
  ...
  function pushRobotClose(issue) {
    appendMsg({
      ...
      avatar: ROBOT.avatar,
      robot: true,
      html: '<div class="k3-msg__bubble--line">======封盘线======<br>=====停止战斗=====</div>'
    });
    setTimeout(() => {
      appendMsg({
        name: ROBOT.name,
        avatar: ROBOT.avatar,
        robot: true,
        html: `<div class="k3-msg__bubble--list"><pre>${buildBetListVerifyText(issue)}</pre></div>`
      });
    }, 500);
  }
  ...
  function pushRobotResult(nums, issue) {
    ...
    setTimeout(() => {
      appendMsg({
        ...
        avatar: ROBOT.avatar,
        robot: true,
        html: `<div class="k3-msg__bubble--list"><pre>${buildWinListVerifyText(issue, settled)}</pre></div>`
      });
    }, 1500);
  }
  ...
  function initKeypad() {
    ...
    for (let r = 0; r < 4; r++) {
      ...
      mid[r].forEach(k => {
        html += `<button type="button" class="k3-key" data-key="${k}">${k}</button>`;
      });
      const rk = right[r];
      const cls = rk === '删除' ? ' k3-key--del' : ' k3-key--side';
  ...
  $('#playSheetBody').html(PLAY_ITEMS.map(p =>
    `<button type="button" class="k3-play-item" data-play="${p}">${p}</button>`
  ).join(''));
  ...
  $('#playSheetBody').on('click', '.k3-play-item', function () {
  ...
  $(document).on('click', function (e) {
    if (!$(e.target).closest('#keypad, #chatInput, .k3-composer').length && $('#keypad').is(':visible')) {
      if (!$(e.target).closest('.k3-plus-menu, #plusBtn, .k3-menu-mask').length) {
        showKeypad(false);
      }
    }
  });
