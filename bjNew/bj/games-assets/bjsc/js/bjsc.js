 ...
 */
$(function () {
  const BET_SEC = 60;         // 下注 1 分钟
  const FREEZE_SEC = 20;      // 封盘 20 秒
  const DRAW_SEC = 10;        // 开奖 10 秒
  const INTERVAL = BET_SEC + FREEZE_SEC + DRAW_SEC; // 每期 90 秒
  ...
  function fmtIssue8(seq) {
    return '3403' + pad(Math.max(1, seq) % 10000, 4);
  }
  function fmtTime(d) {
  ...
  function ballHtml(n, cls) {
    ...
    const c = cls ? ` ${cls}` : '';
    return `<img class="bjsc-ball-img${c}" src="./assets/balls/ball-${num}.png" alt="${n}" loading="lazy">`;
  }

  function getStats(uid) {
    const key = 'bjsc_stats_' + uid;
    let s = null;
    try { s = JSON.parse(localStorage.getItem(key)); } catch (e) { /* */ }
    return s || { turnover: 0, winloss: 0, rebate: 0 };
  }
  function saveStats(uid, s) {
    localStorage.setItem('bjsc_stats_' + uid, JSON.stringify(s));
  }
  ...
  function appendMsg(opts) {
    ...
    const avatar = `<div class="bjsc-msg__avatar"><img src="${fixAvatar(opts.avatar)}" alt=""></div>`;
    const meta = `<div class="bjsc-msg__meta">
            <span class="bjsc-msg__name">${opts.name}</span>
            <span class="bjsc-msg__time">${time}</span>
          </div>`;
    const bubble = `<div class="bjsc-msg__bubble">${opts.html}</div>`;
    const html = isSelf
      ? `<div class="bjsc-msg is-self">
        <div class="bjsc-msg__top">${meta}${avatar}</div>
        ${bubble}
      </div>`
      : `<div class="bjsc-msg${isRobot ? ' is-robot' : ''}">
        ${avatar}
        <div class="bjsc-msg__body">
          ${meta}
          ${bubble}
        </div>
      </div>`;
  ...
  function resultCardHtml(nums, issue) {
    ...
    const lh = lhRow(nums);
    const top3 = [
      { n: nums[1], label: '亚军', rank: 2 },
      { n: nums[0], label: '冠军', rank: 1 },
      { n: nums[2], label: '季军', rank: 3 }
    ];
    const podium = top3.map(item => `
      <div class="bjsc-podium-slot is-r${item.rank}">
        <span class="bjsc-podium-slot__label">${item.label}</span>
        <div class="bjsc-podium-slot__shield">
          ${ballHtml(item.n, 'podium')}
        </div>
      </div>
    `).join('');
    return `
      <div class="bjsc-result-card">
        <div class="bjsc-result-card__head">
          <span class="bjsc-result-card__title">北京赛车</span>
          <div class="bjsc-result-card__order">${nums.map(n => ballHtml(n, 'sm')).join('')}</div>
        </div>
        <div class="bjsc-result-card__stage">
          <div class="bjsc-result-card__stage-bg" aria-hidden="true"></div>
          <div class="bjsc-result-card__podium">${podium}</div>
        </div>
        <div class="bjsc-result-card__foot">
          <div class="bjsc-result-stat">
  ...
  function pushRobotClose(issue) {
    appendMsg({
      ...
      avatar: ROBOT.avatar,
      robot: true,
      html: '<div class="bjsc-msg__bubble--line">======封盘线======<br>=====停止战斗=====</div>'
    });
    setTimeout(() => {
      appendMsg({
        name: ROBOT.name,
        avatar: ROBOT.avatar,
        robot: true,
        html: `<div class="bjsc-msg__bubble--list"><pre>${buildBetListVerifyText(issue)}</pre></div>`
      });
    }, 500);
  }
  ...
  function pushRobotResult(nums, issue) {
    ...
    setTimeout(() => {
      appendMsg({
        ...
        avatar: ROBOT.avatar,
        robot: true,
        html: `<div class="bjsc-msg__bubble--list"><pre>${buildWinListVerifyText(issue, settled)}</pre></div>`
      });
    }, 1500);
  }
  ...
  function initKeypad() {
    ...
    for (let r = 0; r < 4; r++) {
      ...
      mid[r].forEach(k => {
        html += `<button type="button" class="bjsc-key" data-key="${k}">${k}</button>`;
      });
      const rk = right[r];
      const cls = rk === '删除' ? ' bjsc-key--del' : (rk === '冠亚和' ? ' bjsc-key--gy' : ' bjsc-key--side');
  ...
  $('#playSheetBody').html(PLAY_ITEMS.map(p =>
    `<button type="button" class="bjsc-play-item" data-play="${p}">${p}</button>`
  ).join(''));
  ...
  $('#playSheetBody').on('click', '.bjsc-play-item', function () {
  ...
  $('#keyGrid').on('click', '.bjsc-key', function () {
  ...
  $(document).on('click', function (e) {
    if (!$(e.target).closest('#keypad, #chatInput, .bjsc-composer').length && $('#keypad').is(':visible')) {
      if (!$(e.target).closest('.bjsc-plus-menu, #plusBtn, .bjsc-menu-mask').length) {
        showKeypad(false);
      }
    }
  });
