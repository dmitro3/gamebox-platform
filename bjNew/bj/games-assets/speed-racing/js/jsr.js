 ...
 */
$(function () {
  const BET_SEC = 40;         // 下注 40 秒
  const FREEZE_SEC = 15;      // 封盘 15 秒
  const DRAW_SEC = 5;         // 开奖 5 秒
  const INTERVAL = BET_SEC + FREEZE_SEC + DRAW_SEC; // 每期 60 秒
  ...
  function ballHtml(n, cls) {
    ...
    const c = cls ? ` ${cls}` : '';
    return `<img class="jsr-ball-img${c}" src="./assets/balls/ball-${num}.png" alt="${n}" loading="lazy">`;
  }

  function getStats(uid) {
    const key = 'jsr_stats_' + uid;
    let s = null;
    try { s = JSON.parse(localStorage.getItem(key)); } catch (e) { /* */ }
    return s || { turnover: 0, winloss: 0, rebate: 0 };
  }
  function saveStats(uid, s) {
    localStorage.setItem('jsr_stats_' + uid, JSON.stringify(s));
  }
  ...
  function appendMsg(opts) {
    ...
    const avatar = `<div class="jsr-msg__avatar"><img src="${fixAvatar(opts.avatar)}" alt=""></div>`;
    const meta = `<div class="jsr-msg__meta">
            <span class="jsr-msg__name">${opts.name}</span>
            <span class="jsr-msg__time">${time}</span>
          </div>`;
    const bubble = `<div class="jsr-msg__bubble">${opts.html}</div>`;
    const html = isSelf
      ? `<div class="jsr-msg is-self">
        <div class="jsr-msg__top">${meta}${avatar}</div>
        ${bubble}
      </div>`
      : `<div class="jsr-msg${isRobot ? ' is-robot' : ''}">
        ${avatar}
        <div class="jsr-msg__body">
          ${meta}
          ${bubble}
        </div>
      </div>`;
  ...
  function resultCardHtml(nums, issue) {
    ...
    const podium = top3.map(item => `
      <div class="jsr-podium-slot is-r${item.rank}">
        <span class="jsr-podium-slot__label">${item.label}</span>
        <div class="jsr-podium-slot__shield">
          ${ballHtml(item.n, 'podium')}
        </div>
      </div>
    `).join('');
    return `
      <div class="jsr-result-card">
        <div class="jsr-result-card__head">
          <span class="jsr-result-card__title">极速赛车</span>
          <div class="jsr-result-card__order">${nums.map(n => ballHtml(n, 'sm')).join('')}</div>
        </div>
        <div class="jsr-result-card__stage">
          <div class="jsr-result-card__stage-bg" aria-hidden="true"></div>
          <div class="jsr-result-card__podium">${podium}</div>
        </div>
        <div class="jsr-result-card__foot">
          <div class="jsr-result-stat">
  ...
  function pushRobotClose(issue) {
    appendMsg({
      ...
      avatar: ROBOT.avatar,
      robot: true,
      html: '<div class="jsr-msg__bubble--line">======封盘线======<br>=====停止战斗=====</div>'
    });
    setTimeout(() => {
      appendMsg({
        name: ROBOT.name,
        avatar: ROBOT.avatar,
        robot: true,
        html: `<div class="jsr-msg__bubble--list"><pre>${buildBetListVerifyText(issue)}</pre></div>`
      });
    }, 500);
  }
  ...
  function pushRobotResult(nums, issue) {
    ...
    setTimeout(() => {
      appendMsg({
        ...
        avatar: ROBOT.avatar,
        robot: true,
        html: `<div class="jsr-msg__bubble--list"><pre>${buildWinListVerifyText(issue, settled)}</pre></div>`
      });
    }, 1500);
  }
  ...
  function initKeypad() {
    ...
    for (let r = 0; r < 4; r++) {
      ...
      mid[r].forEach(k => {
        html += `<button type="button" class="jsr-key" data-key="${k}">${k}</button>`;
      });
      const rk = right[r];
      const cls = rk === '删除' ? ' jsr-key--del' : (rk === '冠亚和' ? ' jsr-key--gy' : ' jsr-key--side');
  ...
  $('#playSheetBody').html(PLAY_ITEMS.map(p =>
    `<button type="button" class="jsr-play-item" data-play="${p}">${p}</button>`
  ).join(''));
  ...
  $('#playSheetBody').on('click', '.jsr-play-item', function () {
  ...
  $('#keyGrid').on('click', '.jsr-key', function () {
  ...
  $(document).on('click', function (e) {
    if (!$(e.target).closest('#keypad, #chatInput, .jsr-composer').length && $('#keypad').is(':visible')) {
      if (!$(e.target).closest('.jsr-plus-menu, #plusBtn, .jsr-menu-mask').length) {
        showKeypad(false);
      }
    }
  });
