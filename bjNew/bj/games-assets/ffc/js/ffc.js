...
$(function () {
  ...
  function ballHtml(n, cls) {
    const c = cls ? ' ' + cls : '';
    return `<span class="ffc-ball-img ffc-ball-digit${c}">${n}</span>`;
  }
  ...
  function resultCardHtml(nums, issue) {
    ...
    return `<div class="ffc-result-card"><div class="ffc-result-card__head"><span class="ffc-result-card__title">分分彩</span>
      <div class="ffc-result-card__order">${nums.map(n=>ballHtml(n,'sm')).join('')}</div></div>
      <div class="ffc-result-card__foot"><div class="ffc-result-stat"><span class="k">期号</span><span class="v">No.${String(issue).slice(-6)}</span></div>
      <div class="ffc-result-stat"><span class="k">和值</span><span class="v">${meta.text}</span></div>
      <div class="ffc-result-stat"><span class="k">形态</span><span class="v">${lhRow(nums)}</span></div></div></div>`;
  }
  ...
  function fmtIssue8(seq) {
    return '3410' + pad(Math.max(1, seq) % 10000, 4);
  }
  function fmtTime(d) {
  ...
  function appendMsg(opts) {
    ...
    const avatar = `<div class="ffc-msg__avatar"><img src="${fixAvatar(opts.avatar)}" alt=""></div>`;
    const meta = `<div class="ffc-msg__meta">
            <span class="ffc-msg__name">${opts.name}</span>
            <span class="ffc-msg__time">${time}</span>
          </div>`;
    const bubble = `<div class="ffc-msg__bubble">${opts.html}</div>`;
    const html = isSelf
  ...
  function pushRobotClose(issue) {
    appendMsg({
      ...
      avatar: ROBOT.avatar,
      robot: true,
      html: '<div class="ffc-msg__bubble--line">======封盘线======<br>=====停止战斗=====</div>'
    });
    setTimeout(() => {
      appendMsg({
        name: ROBOT.name,
        avatar: ROBOT.avatar,
        robot: true,
        html: `<div class="ffc-msg__bubble--list"><pre>${buildBetListVerifyText(issue)}</pre></div>`
      });
    }, 500);
  }
  ...
  function pushRobotResult(nums, issue) {
    ...
    setTimeout(() => {
      appendMsg({
        ...
        avatar: ROBOT.avatar,
        robot: true,
        html: `<div class="ffc-msg__bubble--list"><pre>${buildWinListVerifyText(issue, settled)}</pre></div>`
      });
    }, 1500);
  }
  ...
  function initKeypad() {
    ...
    let html = '';
    for (let r = 0; r < 4; r++) {
      html += `<button type="button" class="ffc-key ffc-key--side" data-key="${left[r]}">${left[r]}</button>`;
      mid[r].forEach(k => {
        html += `<button type="button" class="ffc-key" data-key="${k}">${k}</button>`;
      });
      const rk = right[r];
      const cls = rk === '删除' ? ' ffc-key--del' : ' ffc-key--side';
      html += `<button type="button" class="ffc-key${cls}" data-key="${rk}">${rk}</button>`;
    }
    $('#keyGrid').html(html);
  }
  ...
  $('#playSheetBody').html(PLAY_ITEMS.map(p =>
    `<button type="button" class="ffc-play-item" data-play="${p}">${p}</button>`
  ).join(''));
  ...
  $('#playSheetBody').on('click', '.ffc-play-item', function () {
