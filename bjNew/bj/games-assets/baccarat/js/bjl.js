...
$(function () {
  ...
  function cardSpan(c, cls) {
    return `<span class="bjl-card ${c.red?'is-red':''} ${cls||''}">${c.text}</span>`;
  }
  ...
  function renderDrawBar(row) {
    ...
    $('#barBalls').html(`<span class="bjl-bjl">庄${row.meta.bv} 闲${row.meta.pv}</span>`);
    $('#barGy').text(row.gy);
  }
  ...
  function resultCardHtml(draw, issue) {
    ...
    return `<div class="bjl-result-card"><div class="bjl-result-card__head"><span class="bjl-result-card__title">百家乐</span>
      <div class="bjl-result-card__order"><span class="bjl-bjl">庄${m.bv} : 闲${m.pv}</span></div></div>
      <div class="bjl-result-card__foot"><div class="bjl-result-stat"><span class="k">期号</span><span class="v">No.${String(issue).slice(-6)}</span></div>
      <div class="bjl-result-stat"><span class="k">结果</span><span class="v">${m.result}</span></div>
      <div class="bjl-result-stat"><span class="k">庄闲</span><span class="v">${m.bv} / ${m.pv}</span></div></div></div>`;
  }
  ...
  function fmtIssue8(seq) {
    return '3409' + pad(Math.max(1, seq) % 10000, 4);
  }
  function fmtTime(d) {
  ...
  function appendMsg(opts) {
    ...
    const avatar = `<div class="bjl-msg__avatar"><img src="${fixAvatar(opts.avatar)}" alt=""></div>`;
    const meta = `<div class="bjl-msg__meta">
            <span class="bjl-msg__name">${opts.name}</span>
            <span class="bjl-msg__time">${time}</span>
          </div>`;
    const bubble = `<div class="bjl-msg__bubble">${opts.html}</div>`;
    const html = isSelf
      ? `<div class="bjl-msg is-self">
        <div class="bjl-msg__top">${meta}${avatar}</div>
        ${bubble}
      </div>`
      : `<div class="bjl-msg${isRobot ? ' is-robot' : ''}">
        ${avatar}
        <div class="bjl-msg__body">
          ${meta}
          ${bubble}
        </div>
      </div>`;
  ...
  function pushRobotClose(issue) {
    appendMsg({
      ...
      avatar: ROBOT.avatar,
      robot: true,
      html: '<div class="bjl-msg__bubble--line">======封盘线======<br>=====停止战斗=====</div>'
    });
    setTimeout(() => {
      appendMsg({
        name: ROBOT.name,
        avatar: ROBOT.avatar,
        robot: true,
        html: `<div class="bjl-msg__bubble--list"><pre>${buildBetListVerifyText(issue)}</pre></div>`
      });
    }, 500);
  }
  ...
  function pushRobotResult(nums, issue) {
    ...
    setTimeout(() => {
      appendMsg({
        ...
        avatar: ROBOT.avatar,
        robot: true,
        html: `<div class="bjl-msg__bubble--list"><pre>${buildWinListVerifyText(issue, settled)}</pre></div>`
      });
    }, 1500);
  }
  ...
  function initKeypad() {
    ...
    let html = '';
    for (let r = 0; r < 4; r++) {
      html += `<button type="button" class="bjl-key bjl-key--side" data-key="${left[r]}">${left[r]}</button>`;
      mid[r].forEach(k => {
        html += `<button type="button" class="bjl-key" data-key="${k}">${k}</button>`;
      });
      const rk = right[r];
      const cls = rk === '删除' ? ' bjl-key--del' : ' bjl-key--side';
      html += `<button type="button" class="bjl-key${cls}" data-key="${rk}">${rk}</button>`;
    }
    $('#keyGrid').html(html);
  }
  ...
  $('#playSheetBody').html(PLAY_ITEMS.map(p =>
    `<button type="button" class="bjl-play-item" data-play="${p}">${p}</button>`
  ).join(''));
  ...
  $('#playSheetBody').on('click', '.bjl-play-item', function () {
  ...
  $('#keyGrid').on('click', '.bjl-key', function () {
  ...
  $(document).on('click', function (e) {
    if (!$(e.target).closest('#keypad, #chatInput, .bjl-composer').length && $('#keypad').is(':visible')) {
      if (!$(e.target).closest('.bjl-plus-menu, #plusBtn, .bjl-menu-mask').length) {
        showKeypad(false);
      }
    }
  });
